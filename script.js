...
function renderCity(){
  ...
  if(buyMerchantBtn) buyMerchantBtn.addEventListener('click', ()=>{
    ...
    confirmAction({icon:'🛒', title:'Buy from Merchant', desc:`Spend 🪙${price} gold on this ${BUILDING_TYPES[buildingKey].name} skin? The merchant only offers one purchase per visit.`, confirmLabel:`🪙${price} · Buy`, onConfirm:()=>{
      state.gold -= price;
      if(!state.skins[buildingKey]) state.skins[buildingKey] = [0];
      if(!state.skins[buildingKey].includes(skinIdx)) state.skins[buildingKey].push(skinIdx);
...
function renderShop(){
  ...
  wrap.querySelectorAll('[data-buylegendary]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      confirmAction({icon:'👑', title:'Buy Legendary Skin', desc:`Spend 💎30 gems to unlock the legendary ${BUILDING_TYPES[key].name} skin?`, confirmLabel:'💎30 · Buy', onConfirm:()=>{
        state.gems -= 30;
        if(!state.skins[key]) state.skins[key] = [0];
        state.skins[key].push(3);
        state.equippedSkin[key] = 3;
        toast(`👑 Legendary ${BUILDING_TYPES[key].name} skin unlocked!`);
        renderAll();
      }});
    });
  });
  wrap.querySelectorAll('[data-buyskin]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      confirmAction({icon:'🎨', title:'Buy Skin', desc:`Spend 🪙20 gold to unlock a new ${def.name} skin?`, confirmLabel:'🪙20 · Buy', onConfirm:()=>{
        state.gold-=20;
        state.skins[key].push(unowned);
        state.equippedSkin[key]=unowned;
        toast(`${def.name} skin unlocked!`);
        renderAll();
      }});
    });
  });
...
/* ============================= PLAYER MARKET ============================= *
 * All trading logic lives server-side (server/server.js) — this panel just
...
async function renderLeaderboard(){
  ...
  wrap.innerHTML = `
    <div class="panel">
      ...
      <input type="text" id="lbSearchInput" placeholder="🔍 Find a player by ID (e.g. pi_username) — not just top 50" value="${leaderboardSearchQuery}" style="width:100%; box-sizing:border-box; margin:8px 0; padding:8px 10px; border-radius:8px; border:1px solid var(--gold); background:transparent; color:var(--text); font-size:13px;" />
  ...
  if(!API_BASE){
    listEl.innerHTML = `<p class="panel-sub">Rankings need the backend in server/ deployed first (API_BASE is empty).</p>`;
    return;
  }
  try{
...
function renderLeaderboardList(){
  ...
  if(isSearching){
    ...
    pool = players.filter(p=> p.playerId.toLowerCase().includes(q));
  ...
  listEl.innerHTML = sorted.map((p)=>{
    ...
    const name = p.playerId.replace(/^pi_|^guest_/,'') + (p.playerId.startsWith('guest_') ? ' (guest)' : '');
  ...
  if(isSearching && sorted.length===0){
    listEl.innerHTML = `<p class="panel-sub">No player found matching "${leaderboardSearchQuery}".</p>`;
  }
...
function renderGiftHistory(){
  ...
  wrap.innerHTML = `<div class="gift-history">${entries.map(e=>{
    const who = (e.withPlayerId||'').replace(/^pi_|^guest_/,'');
...
async function sendGift(recipientId, kind, itemId, amount){
  ...
  try{
    const res = await fetch(`${API_BASE}/gift/send`, {
      ...
      body: JSON.stringify({ senderId:getPlayerId(), recipientId, kind, itemId, amount, message }),
    });
    ...
    logGiftHistory({ dir:'sent', withPlayerId:recipientId, label, message });
    toast('🎁 Gift sent!');
...
function applyPendingGifts(){
  ...
  gifts.forEach(g=>{
    const fromName = (g.fromPlayerId||'').replace(/^pi_|^guest_/,'');
    let label = '';
    if(g.kind==='gold'){
      state.gold = (state.gold||0) + g.amount;
      label = `🪙${g.amount}`;
      toast(`🎁 ${fromName} sent you 🪙${g.amount}!`);
    } else if(g.kind==='badge'){
      state.ownedBadges = state.ownedBadges || [];
      const b = findBadge(g.itemId);
      if(!state.ownedBadges.includes(g.itemId)) state.ownedBadges.push(g.itemId);
      ...
      if(!state.ownedFrames.includes(g.itemId)) state.ownedFrames.push(g.itemId);
      label = f ? `${f.name} frame` : g.itemId;
      toast(`🎁 ${fromName} sent you the ${label}!`);
    }
    if(g.message) toast(`💌 "${g.message}"`);
    logGiftHistory({ dir:'received', withPlayerId:g.fromPlayerId, label, message:g.message });
  });
  state.pendingGifts = [];
...
async function renderMarketplacePanel(){
  ...
  if(!API_BASE){
    listArea.innerHTML = `<p class="panel-sub">No market server configured yet (API_BASE is empty) — the Player Market needs the backend in server/ deployed first.</p>`;
    mineArea.innerHTML = '';
    return;
  }
      ...
      confirmAction({icon:'🛒', title:'Buy Listing', desc:`Buy ${name} for 🪙${price} gold from this player's listing?`, confirmLabel:`🪙${price} · Buy`, onConfirm: async ()=>{
        ...
        try{
          const res = await fetch(`${API_BASE}/market/buy`, {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ listingId: btn.dataset.buylisting, buyerId: myId }),
          });
          const data = await res.json();
      ...
      confirmAction({icon:'🗑️', title:'Cancel Listing', desc:`Cancel your listing for ${name}? The skin will be returned to you.`, confirmLabel:'Cancel Listing', onConfirm: async ()=>{
        ...
        try{
          const res = await fetch(`${API_BASE}/market/cancel`, {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ listingId: btn.dataset.cancellisting, sellerId: myId }),
          });
...
async function listSkinForSale(buildingKey, skinIdx, price){
  ...
  try{
    const res = await fetch(`${API_BASE}/market/list`, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ sellerId: getPlayerId(), buildingKey, skinIdx, price }),
    });
    const data = await res.json();
...
/* ============================= PI NETWORK INTEGRATION =============================
   ...
   pass — see purchaseWithPi() above and server/server.js's payment endpoints. */
...
function onIncompletePaymentFound(payment){
  ...
  if(payment && payment.identifier){
    fetch(`${API_BASE}/payments/complete`, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ paymentId: payment.identifier, txid: payment.transaction && payment.transaction.txid, playerId: getPlayerId() }),
 ...
 * that's actually GRANTED happens server-side in server/server.js, using
 ...
 * See server/README.md before accepting real payments with this.
 * ------------------------------------------------------------------------- */
function purchaseWithPi(amount, memo, metadata){
  ...
  if(!API_BASE){
    toast('This build has no payment server configured yet (API_BASE is empty).');
    return;
  }
  Pi.createPayment({ amount, memo, metadata }, {
    onReadyForServerApproval: (paymentId)=>{
      fetch(`${API_BASE}/payments/approve`, {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ paymentId, playerId: getPlayerId() }),
      }).catch(e=>console.error('Approve request failed:', e));
    },
    onReadyForServerCompletion: (paymentId, txid)=>{
      fetch(`${API_BASE}/payments/complete`, {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ paymentId, txid, playerId: getPlayerId() }),
      })
        .then(r=>r.json())
        .then(data=>{
...
function buyBadge(id, currency){
  ...
  confirmAction({icon:b.icon, title:`Buy ${b.name}`, desc:`Spend ${label} to unlock ${b.icon} ${b.name}?`, confirmLabel:`${label} · Buy`, onConfirm:()=>{
    if(currency==='gems') state.gems -= b.gemPrice; else state.gold -= b.goldPrice;
    state.ownedBadges.push(id);
    toast(`${b.icon} ${b.name} unlocked!`);
    checkCollections();
    renderAll();
  }});
}
...
function buyFrame(id, currency){
  ...
  confirmAction({icon:'🖼️', title:`Buy ${f.name}`, desc:`Spend ${label} to unlock ${f.name}?`, confirmLabel:`${label} · Buy`, onConfirm:()=>{
    if(currency==='gems') state.gems -= f.gemPrice; else state.gold -= f.goldPrice;
    state.ownedFrames.push(id);
    toast(`${f.name} unlocked!`);
    renderAll();
  }});
}
...
function buyEffect(id, currency){
  ...
  confirmAction({icon:e.icon, title:`Buy ${e.name}`, desc:`Spend ${label} to unlock ${e.icon} ${e.name}?`, confirmLabel:`${label} · Buy`, onConfirm:()=>{
    if(currency==='gems') state.gems -= e.gemPrice; else state.gold -= e.goldPrice;
    state.ownedEffects.push(id);
    toast(`${e.icon} ${e.name} unlocked!`);
    renderAll();
  }});
}
 ...
 *   2. A tiny backend (see server/ folder) — set API_BASE below once you've
...
// Leave this empty to stay localStorage-only. Once you deploy the server in
// server/, set this to its URL, e.g. "https://kingdoms-words-api.onrender.com/api"
const API_BASE = "";

const LOCAL_KEY_PREFIX = 'kingdomsWords_save_';

function getPlayerId(){
  if(piUser && piUser.username) return 'pi_' + piUser.username;
...
async function saveRemote(s){
  if(!API_BASE) return;
  try{
    await fetch(`${API_BASE}/state/${encodeURIComponent(getPlayerId())}`, {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify(serializeState(s)),
    });
...
async function loadRemote(){
  ...
  try{
    const res = await fetch(`${API_BASE}/state/${encodeURIComponent(getPlayerId())}`);
...
function buyAura(id, currency){
  ...
  confirmAction({icon:'✨', title:`Buy ${a.name}`, desc:`Spend ${label} to unlock ${a.name}?`, confirmLabel:`${label} · Buy`, onConfirm:()=>{
    if(currency==='gems') state.gems -= a.gemPrice; else state.gold -= a.goldPrice;
...
function buySoundPack(id, currency){
  ...
  confirmAction({icon:'🔊', title:`Buy ${p.name}`, desc:`Spend ${label} to unlock ${p.name}?`, confirmLabel:`${label} · Buy`, onConfirm:()=>{
    if(currency==='gems') state.gems -= p.gemPrice; else state.gold -= p.goldPrice;
    state.ownedSoundPacks.push(id);
